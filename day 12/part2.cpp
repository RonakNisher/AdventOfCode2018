...
// 	}
	...
	string generation;
	long p2 = 50000000000;
	int iterationStartMatch = 0;
	int oldHashStart, newHashStart;
    ...
    // 	cout<<i+1<<" : "<<generation<<endl;
    	...
    	if (generation.find(oldGeneration) != string::npos)
    	{
    	    cout<<"found match at iteration "<<i+1<<endl;
    	    
    	    oldHashStart = oldGeneration.find('#');
    	    int oldHashEnd = oldGeneration.rfind('#');
    	    newHashStart = generation.find('#');
    	    int newHashEnd = generation.rfind('#');
    	    
    	    cout<<oldHashStart<<" to "<<newHashStart<<endl;
    	    initialState = generation;
    	    
    	    iterationStartMatch = i;
    	    cout<<"it start match "<<iterationStartMatch<<endl;
	...
	currentPos = initialPotNumber;
	long score2 = 0;
	for (char c : oldGeneration)
	{
	    if (c == '#')
	    {
	        score2+= (currentPos + (p2 - iterationStartMatch)*(newHashStart - oldHashStart));
	    }
	    currentPos++;
	}
