...
struct group 
{
	...
	{
		// cout<<"grp "<<id<<" "<<noOfUnits<<" x "<<getBoostDamage()<<endl;
		effectivePower = noOfUnits * getBoostDamage();
	...
	int calculatedamage(group& g)
	{
		...
		int damageDone = noOfUnits * getBoostDamage() * damageMultiplier;
	}
...
bool immuneGroupAlive(const vector<group>& groups)
{
	bool immuneAlive = false;
...
void fillGroups(vector<group>& groups)
{
	groups.clear();
	...
	// test
	// 		vector<string> g1Immune = {};
	// vector<string> g1Weak = { "radiation", "bludgeoning" };
	// groups.emplace_back(1 /*id*/, 0 /*type*/, 17 /*noOfUnits*/, 5390 /*hitPoint*/, 4507 /*damage*/, 2 /*initiative*/, "fire" /*attackType*/, 
	// 					g1Weak /*weakTo*/, g1Immune /*immuneTo*/);
						
	// vector<string> g2Immune = { "fire" };
	// vector<string> g2Weak = { "bludgeoning" , "slashing" };
	// groups.emplace_back(2 /*id*/, 0 /*type*/, 989 /*noOfUnits*/, 1274 /*hitPoint*/, 25 /*damage*/, 3 /*initiative*/, "slashing" /*attackType*/, 
	// 					g2Weak /*weakTo*/, g2Immune /*immuneTo*/);
						
	// vector<string> g3Immune = {};
	// vector<string> g3Weak = { "radiation" };
	// groups.emplace_back(3 /*id*/, 1 /*type*/, 801 /*noOfUnits*/, 4706 /*hitPoint*/, 116 /*damage*/, 1 /*initiative*/, "bludgeoning" /*attackType*/, 
	// 					g3Weak /*weakTo*/, g3Immune /*immuneTo*/);
						
	// vector<string> g4Immune = { "radiation" };
	// vector<string> g4Weak = { "fire", "cold" };
	// groups.emplace_back(4 /*id*/, 1 /*type*/, 4485 /*noOfUnits*/, 2961 /*hitPoint*/, 12 /*damage*/, 4 /*initiative*/, "slashing" /*attackType*/, 
	// 					g4Weak /*weakTo*/, g4Immune /*immuneTo*/);
}

void resetGroups(vector<group>& groups, int boost)
{
	fillGroups(groups);
	for (group& gr : groups)
	{
...
{ 
	...
	{
		safetyEscape = false;
		resetGroups(groups, boost);
		groupsStillAlive.clear();
		...
		int i = 0;
		while (areBothGroupsAlive(groups) && !safetyEscape)
		{
			// printStats(groups);
			...
			for (const int i : groupsStillAlive)
			{
				...
				if (attackingGroup->target == -1 || !attackingGroup->isAlive)
					continue;
				...
				{
					// int newUnits = (defendingGroupEffectiveHealth - damageToBeDone)/defendingGroup->hitPoint;
					int noOfUnitsKilled = damageToBeDone / defendingGroup->hitPoint;
					if (noOfUnitsKilled == 0)
					{
						// cout<<defendingGroup->hitPoint<<", newUnits "<<newUnits<<endl;
						// defendingGroup->noOfUnits = newUnits;
						// if (newUnits == 0)
						// 	defendingGroup->isAlive = false;
						// cout<<"safetyEscape "<<endl;
						// safetyEscape = true;
					}
					else
					{
